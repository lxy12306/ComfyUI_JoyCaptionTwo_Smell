...
class JoyImageAdapter:
    def __init__(self, base_model_path, load_device, offload_device):
        ...
        # Image Adapter
        adapter_path = os.path.join(self.joy_caption_two_path, "image_adapter.pt")
...
class JoyLLM:
    def __init__(self, base_model_path, load_device, offload_device, model_id):
        self.load_device = load_device
        self.offload_device = offload_device
        self.type = text_encoder_dtype()
        ...
        tokenizer = AutoTokenizer.from_pretrained(os.path.join(self.joy_caption_two_path, "text_model"), use_fast=True)
    ...
    def load_llm_model(self):
        if self.text_model is None or self.current_model_id != self.model_id:
            ...
            LLM_PATH = load_hg_model(self.model_id, self.base_model_path, "LLM")
            text_model_path = os.path.join(self.joy_caption_two_path, "text_model")
...
class JoyTwoPipeline:
    ...
    def loadLLM(self, model_id):
        if self.llm is None:
            self.llm = JoyLLM(self.base_model_path, self.load_device, self.offload_device, model_id)
        else:
